...
11. **Always build a page template first** — every website needs a root template at `src/templates/<ModuleName>Template/default.server.tsx`. Use `jahiaComponent({ componentType: 'template', nodeType: 'jnt:page', name: 'default' }, ({ 'jcr:title': title }, { renderContext }) => ...)`. It must include: a `<header>` with site navigation (`AbsoluteArea`), a `<main>` with content Areas, and a `<footer>` (`AbsoluteArea`). Include a `<title>` tag using the page's `jcr:title` prop for SEO. Build and deploy the page template before any page-specific components.
